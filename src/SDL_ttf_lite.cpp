...
TTF_Font* TTF_OpenFontIndexRW( SDL_RWops *src, int freesrc, int ptsize, long index )
{
	...
	if ( FT_IS_SCALABLE(face) ) {
	  ...
	  font->height  = font->ascent - font->descent + 1; // 1 = baseline;
...
void TTF_FreeGlyph(TTF_Glyph* data){
  if(!data) return;
...
TTF_Glyph *TTF_RenderGlyph_Blended(TTF_Font *font, Uint16 ch)
{
	Uint32 alpha;
	Uint32 pixel;
	Uint8 *src;
  ...
  raster = NULL;
	...
	if(error = Find_Glyph(font, ch, CACHED_METRICS|CACHED_PIXMAP)) return NULL;
  ...
  raster = (TTF_Glyph*)malloc(sizeof(TTF_Glyph));
	if (raster==NULL){
    free((void*)raster);
    return NULL;
  }
  raster->pixels = NULL;
  raster->pixels = (Uint32*)malloc(sizeof(Uint32) * glyph->pixmap.width * row);
  if (raster->pixels==NULL){
    free((void*)raster->pixels);
    return NULL;
  }
  
  raster->h = glyph->pixmap.rows;
  raster->w = glyph->pixmap.width;
  raster->y_offset = glyph->yoffset;
  
  pitch = glyph->pixmap.width * sizeof(Uint32);
